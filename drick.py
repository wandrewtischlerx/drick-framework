...
def cbanner():
    try:
        ...
        if port == 21:
            # FTP banner
            banner = s.recv(1024)
            print(f"\033[32m\nBanner FTP capturado na porta {port}\033[0;0m\n{banner.decode()}\n")
        
        elif port == 22:
            # SSH banner
            banner = s.recv(1024)
            print(f"\033[32m\nBanner SSH capturado na porta {port}\033[0;0m\n{banner.decode()}\n")
        
        elif port == 23:
            # Telnet banner
            banner = s.recv(1024)
            print(f"\033[32m\nBanner Telnet capturado na porta {port}\033[0;0m\n{banner.decode()}\n")
        
        elif port == 80:
            # HTTP banner
            s.send(b'GET / HTTP/1.1\r\nHost: {}\r\n\r\n'.format(host).encode())
            banner = s.recv(1024)
            print(f"\033[32m\nBanner HTTP capturado na porta {port}\033[0;0m\n{banner.decode()}\n")
        
        elif port == 443:
            # HTTPS banner
            s.send(b'GET / HTTP/1.1\r\nHost: {}\r\n\r\n'.format(host).encode())
            banner = s.recv(1024)
            print(f"\033[32m\nBanner HTTPS capturado na porta {port}\033[0;0m\n{banner.decode()}\n")
        
        elif port == 445:
            # SMB banner
            banner = s.recv(1024)
            print(f"\033[32m\nBanner SMB capturado na porta {port}\033[0;0m\n{banner.decode()}\n")
        
        elif port == 3306:
            # MySQL banner
            banner = s.recv(1024)
            print(f"\033[32m\nBanner MySQL capturado na porta {port}\033[0;0m\n{banner.decode()}\n")
        
        else:
            # Banner padrão para outras portas
            banner = s.recv(1024)
            print(f"\033[32m\nBanner capturado na porta {port}\033[0;0m\n{banner.decode()}\n")
    
    except Exception as e:
        print(f"Falha ao capturar banner na porta {port}: {e}")
...
def geo_ip():
    try:
        ...
        # Função para extrair o domínio básico do host
        def extract_domain(host):
            host = re.sub(r'^https?://', '', host)  # Remove http:// ou https:// do início
            host = host.split('/')[0]  # Remove qualquer caminho ou parâmetro de consulta
            return host
        ...
        for key in ["País", "Estado", "Cidade", "Organização", "ASN", "Coordenadas"]:
            ...
            if data:
                if key == "Coordenadas":
                    info_list.append(f"{key}: \033[32m{data}\033[0;0m")
                    # Construir link do Google Maps com as coordenadas
                    lat, lon = data.split(',')
                    google_maps_link = f"https://www.google.com/maps?q={lat},{lon}"
                    info_list.append(f"Link do Google Maps: \033[32m{google_maps_link}\033[0;0m")
                else:
                    info_list.append(f"{key}: \033[32m{data}\033[0;0m")
        ...
        try:
            ...
            for link in soup.find_all('a', href=True):
                ...
                if href.startswith('/') or base_url in href:
                    ...
                    # Adicionar somente se for uma URL válida
                    if urlparse(directory_url).scheme in ['http', 'https']:
...
print("\n\n############  ########  ##### ### ######  ########  ######## # ##  #####")
print ("####### ### ### ##### --|", nomep, "|-- #########\033[36m v[0.1] \033[0;0m#########")
print("######## ####  ####### ############ ######## # ## #####  # # #  #### ### \n\n")
